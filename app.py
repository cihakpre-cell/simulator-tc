...
if st.session_state.tmy_df is not None and df_char is not None:
    ...
    def generate_pdf_v49():
        ...
        # --- NOVÁ SEKCE: MEZERA A METODIKA ---
        pdf.ln(10)
        pdf.set_font(pdf.font_family, "B", 11)
        pdf.cell(0, 8, cz("METODIKA VÝPOČTU A LOGIKA SIMULACE"), ln=True)
        pdf.set_font(pdf.font_family, "", 9)
        metodika_text = (
            "Výpočet vychází z hodinové simulace energetické bilance objektu (8760 kroků za rok). "
            "Simulace využívá klimatická data TMY (Typický Meteorologický Rok) pro konkrétní GPS lokaci. "
            "Výpočet zohledňuje tepelnou setrvačnost budovy, dynamické řízení teploty otopné vody dle ekvitermy "
            "a prioritní ohřev TUV. Výsledkem je přesné stanovení bodu bivalence a reálného sezónního COP."
        )
        ...
        try:
            ...
            if m_resp.status_code == 200:
                ...
                pdf.image(f_map_name, x=145, y=curr_y_params, w=50) # Umístění vpravo u parametrů
        except:
            pass 
