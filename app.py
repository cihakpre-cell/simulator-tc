import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
from google.colab import files

# --- 1. NAČTENÍ VSTUPŮ Z EXCELU ---
try:
    xls = pd.ExcelFile('vstupy_TC.xlsx')
    df_zadani = pd.read_excel(xls, 'Zadani').set_index('Parametr')
    df_char = pd.read_excel(xls, 'Charakteristika')
    
    # Parametry načtené přímo z tabulky
    nazev = df_zadani.loc['Nazev_Projektu', 'Hodnota']
    ztrata_celkova = float(df_zadani.loc['Tepelna_Ztrata', 'Hodnota'])
    t_design = float(df_zadani.loc['Navrhova_Teplota', 'Hodnota'])
    pocet_tc = int(df_zadani.loc['Pocet_TC_v_Kaskade', 'Hodnota'])
    investice = float(df_zadani.loc['Investice_CAPEX', 'Hodnota'])
    fakt_ut = float(df_zadani.loc['Spotreba_UT_CZT', 'Hodnota'])
    f_tuv = float(df_zadani.loc['Spotreba_TUV_CZT', 'Hodnota'])
    cena_el_mwh = float(df_zadani.loc['Cena_Elektrina_MWh', 'Hodnota'])
    cena_gj_czt = float(df_zadani.loc['Cena_CZT_GJ', 'Hodnota'])
    servis = float(df_zadani.loc['Servisni_Naklady_Rok', 'Hodnota'])

    # Načtení klimatických dat TMY
    tmy = pd.read_csv('tmy_50.024_14.455_2005_2023.csv', skiprows=17)
    tmy.columns = tmy.columns.str.strip()
    tmy['T2m'] = pd.to_numeric(tmy['T2m'], errors='coerce')
    tmy = tmy.dropna(subset=['T2m']).reset_index(drop=True)
    tmy['T_smooth'] = tmy['T2m'].rolling(window=6, min_periods=1).mean()

    # Kalibrace a výpočet CZT
    q_tuv_avg = (f_tuv / 8760) * 1000
    potreba_ut_teorie = [ztrata_celkova * (20 - t) / (20 - t_design) if t < 20 else 0 for t in tmy['T_smooth']]
    k_oprava = fakt_ut / (sum(potreba_ut_teorie) / 1000)
    naklady_czt_rok = (fakt_ut + f_tuv) * (cena_gj_czt * 3.6)

except Exception as e:
    print(f"Chyba při načítání parametrů z Excelu: {e}")

# --- 2. VÝPOČET BODU BIVALENCE ---
t_biv = -99
for t in np.linspace(15, -15, 500):
    q_need = (ztrata_celkova * (20 - t) / (20 - t_design) * k_oprava) + q_tuv_avg
    p_tc = np.interp(t, df_char['Teplota'], df_char['Vykon_kW']) * pocet_tc
    if p_tc < q_need:
        t_biv = t
        break

# --- 3. ROČNÍ SIMULACE ---
results = []
for t_out, t_smooth in zip(tmy['T2m'], tmy['T_smooth']):
    q_total = max(0, ztrata_celkova * (20 - t_smooth) / (20 - t_design) * k_oprava) + q_tuv_avg
    p_max = np.interp(t_out, df_char['Teplota'], df_char['Vykon_kW']) * pocet_tc
    cop = np.interp(t_out, df_char['Teplota'], df_char['COP'])
    
    q_tc = min(q_total, p_max)
    q_biv = q_total - q_tc
    results.append([t_out, q_total, q_tc, q_biv, q_tc/cop if q_tc > 0 else 0, q_biv/0.98])

df_res = pd.DataFrame(results, columns=['Temp', 'Q_need', 'Q_tc', 'Q_biv', 'El_tc', 'El_biv'])

# --- 4. EKONOMICKÉ VYHODNOCENÍ ---
el_total_mwh = (df_res['El_tc'].sum() + df_res['El_biv'].sum()) / 1000
naklady_tc_rok = el_total_mwh * cena_el_mwh + servis
uspora_rok = naklady_czt_rok - naklady_tc_rok
podil_biv_energie = (df_res['Q_biv'].sum() / df_res['Q_need'].sum()) * 100
navratnost = investice / uspora_rok if uspora_rok > 0 else 0

# --- 5. VIZUALIZACE (4 GRAFY PRO PREZENTACI) ---
fig = plt.figure(figsize=(16, 12))

# A. Výkonová rovnováha a bivalence
ax1 = plt.subplot(2, 2, 1)
t_range = np.linspace(-15, 16, 100)
q_range = [(ztrata_celkova * (20 - t) / (20 - t_design) * k_oprava) + q_tuv_avg for t in t_range]
p_range = [np.interp(t, df_char['Teplota'], df_char['Vykon_kW']) * pocet_tc for t in t_range]
ax1.plot(t_range, q_range, 'r-', lw=2, label='Potřeba domu + TUV')
ax1.plot(t_range, p_range, 'b--', lw=2, label=f'Max. výkon kaskády ({pocet_tc}ks)')
ax1.axvline(t_biv, color='black', ls=':', label=f'Bod bivalence {t_biv:.1f}°C')
ax1.set_title("VÝKONOVÁ CHARAKTERISTIKA", fontweight='bold')
ax1.set_xlabel("Venkovní teplota [°C]"); ax1.set_ylabel("Výkon [kW]"); ax1.legend(); ax1.grid(alpha=0.3)

# B. Histogram četnosti (Zóna bivalence)
ax2 = plt.subplot(2, 2, 2)
n, bins, patches = ax2.hist(tmy['T2m'], bins=np.arange(-15, 20, 1), color='skyblue', edgecolor='white')
for i in range(len(patches)):
    if bins[i] < t_biv: patches[i].set_facecolor('#ff4444')
ax2.set_xlim(-15, 20); ax2.set_title("ČETNOST TEPLOT V ROCE (TMY)", fontweight='bold')
ax2.set_ylabel("Hodin v roce")
ax2.annotate('Zóna bivalence\n(Dohřev elektrokotlem)', xy=(t_biv-2, 50), xytext=(t_biv-10, 400),
             arrowprops=dict(facecolor='red', shrink=0.05), color='red', fontweight='bold')

# C. Porovnání ročních nákladů
ax3 = plt.subplot(2, 2, 3)
ax3.bar(['Stávající CZT', f'TČ ({pocet_tc}ks)'], [naklady_czt_rok, naklady_tc_rok], color=['gray', 'green'])
ax3.set_title("ROČNÍ PROVOZNÍ NÁKLADY [Kč]", fontweight='bold')
for i, v in enumerate([naklady_czt_rok, naklady_tc_rok]):
    ax3.text(i, v + 20000, f"{v:,.0f} Kč", ha='center', fontweight='bold')

# D. Tabulka výsledků
ax4 = plt.subplot(2, 2, 4)
ax4.axis('off')
summary = [
    ["Projekt", nazev],
    ["Počet TČ v kaskádě", f"{pocet_tc} ks"],
    ["Celková investice", f"{investice:,.0f} Kč"],
    ["Bod bivalence", f"{t_biv:.1f} °C"],
    ["Podíl bivalence na energii", f"{podil_biv_energie:.2f} %"],
    ["Roční úspora vs CZT", f"{uspora_rok:,.0f} Kč"],
    ["Doba návratnosti", f"{navratnost:.1f} let"]
]
tbl = ax4.table(cellText=summary, loc='center', cellLoc='left', colWidths=[0.5, 0.4])
tbl.auto_set_font_size(False); tbl.set_fontsize(12); tbl.scale(1, 3)

plt.tight_layout(); plt.show()

# --- 6. EXPORT DO EXCELU ---
output_file = f'Analyza_{pocet_tc}ks_TC.xlsx'
with pd.ExcelWriter(output_file) as writer:
    pd.DataFrame([dict(summary)]).to_excel(writer, sheet_name='Souhrn', index=False)
    df_res.to_excel(writer, sheet_name='Hodinova_Data', index=False)

print(f"Výpočet dokončen pro {pocet_tc}ks TČ. Soubor exportován.")
files.download(output_file)
