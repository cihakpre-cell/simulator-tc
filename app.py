...
if tmy_file and char_file:
    ...
    if tmy_raw is not None and df_char is not None:
        ...
        tmy['month'] = pd.to_numeric(tmy['month'], errors='coerce').fillna(1)
        ...
        for _, row in tmy.iterrows():
            ...
            # Korekce COP (jedna pro topení, jedna pro TUV)
            ...
            # Pokud požadavek na vodu převyšuje limit TČ, musí pomoci bivalence
            if t_v_req > limit_voda_tc or t_tuv_cil > limit_voda_tc:
                q_tc = 0 # Zjednodušeně: TČ v tomto režimu neběží (nebo běží jen bivalence)
            else:
                q_tc = min(q_total, p_max)
            
            q_biv = max(0, q_total - q_tc)
            
            # Výpočet elektřiny (vážený COP mezi UT a TUV)
